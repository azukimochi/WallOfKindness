...
class DashboardPage extends Component {
  ...
  validateInfo = event => {
    ...
    if (
      ...
      this.setState({
        updateStatus:
          "Update aborted. Please fill out all the fields in your profile."
      });
    } else {
      this.validateGifts();
    }
  ...
  validateGifts = () => {
    ...
    copyOfGifts.forEach((gift, index) => {
      if (
        gift[index] === "" ||
        gift[index] === undefined ||
        copyOfCategories[index] === "None"
      ) {
        errNum++;
    ...
    if (errNum === 0) {
      ...
      this.setState({
        updateStatus:
          "Update aborted. Please fill out all item names and categories.  You can only have 5 items at once."
      });
    }
  };
  ...
  updateButtonEffect() {
    ...
    setTimeout(function() {
      updateEffect.classList.remove("running");
    }, 2000);
  }
